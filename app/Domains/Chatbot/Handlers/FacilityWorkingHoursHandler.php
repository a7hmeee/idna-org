...
final readonly class FacilityWorkingHoursHandler implements ChatResponseHandlerInterface
{
    ...
    {
        return $intent === ChatbotIntent::FacilityWorkingHours;
    }
    ...
    public function handle(
        ...
        if ($details === null) {
            return new ChatResponseData(
                message: 'عذرًا، ما لقيت معلومات المرفق.',
                type: 'empty_state',
            );
        }
        ...
        if ($details->workingHours === null) {
            return new ChatResponseData(
                message: "عذرًا، لا تتوفر ساعات عمل لـ {$details->name}.",
                type: 'empty_state',
            );
        }

        $lines = ["ساعات عمل {$details->name}:"];
        $lines[] = $details->workingHours;
