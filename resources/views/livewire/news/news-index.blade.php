<div>
    <x-slot name="title">الأخبار</x-slot>

    <div class="flex items-center justify-between mb-6">
        <div>
            <h1 class="text-2xl font-bold text-text">الأخبار</h1>
            <p class="text-sm text-text-tertiary mt-1">إدارة أخبار البلدية</p>
        </div>
        @can('create', \App\Domains\News\Models\NewsItem::class)
            <a href="{{ route('dashboard.news.create') }}" class="px-4 py-2.5 rounded-xl bg-primary text-white text-sm font-semibold hover:bg-primary-dark transition-colors inline-flex items-center gap-2" wire:navigate>
            ...
            <table class="w-full text-sm">
                <thead>
                    <tr class="border-b border-border bg-municipal-50/50">
                        ...
                        <th class="text-center px-4 py-3 text-xs font-semibold text-text-tertiary">تاريخ النشر</th>
                        <th class="text-left px-4 py-3 text-xs font-semibold text-text-tertiary">الإجراءات</th>
                    </tr>
                </thead>
                <tbody>
                    @forelse ($news as $item)
                        <tr class="border-b border-border last:border-0 hover:bg-municipal-50/30 transition-colors" wire:key="news-{{ $item->id }}">
                            <td class="px-4 py-3">
                                <p class="font-semibold text-text">{{ $item->title_ar }}</p>
                                @if ($item->title_en)
                                    <span class="text-xs text-text-tertiary">{{ $item->title_en }}</span>
                                @endif
                            </td>
                            <td class="px-4 py-3">
                                <span class="text-xs text-text-tertiary">{{ $item->category->label() }}</span>
                            </td>
                            <td class="px-4 py-3 text-center">
                                <span class="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-lg text-xs font-semibold
                                    @if($item->status->value === 'published') bg-success/10 text-success
                                    @elseif($item->status->value === 'draft') bg-warning/10 text-warning
                                    @else bg-municipal-50 text-text-tertiary @endif">
                                    {{ $item->status->label() }}
                                </span>
                            </td>
                            <td class="px-4 py-3 text-center">
                                <span class="text-sm text-text-tertiary">{{ number_format($item->views_count) }}</span>
                            </td>
                            <td class="px-4 py-3 text-center">
                                @can('feature', \App\Domains\News\Models\NewsItem::class)
                                    <button wire:click="toggleFeatured({{ $item->id }})" class="transition-colors @if($item->is_featured) text-yellow-500 @else text-text-tertiary hover:text-yellow-500 @endif">
                                        <i data-lucide="star" class="w-4 h-4"></i>
                                    </button>
                                @else
                                    <span class="text-sm @if($item->is_featured) text-yellow-500 @else text-text-tertiary @endif">
                                        <i data-lucide="star" class="w-4 h-4"></i>
                                    </span>
                                @endcan
                            </td>
                            <td class="px-4 py-3 text-center">
                                <span class="text-xs text-text-tertiary">{{ $item->publish_at ? $item->publish_at->format('Y/m/d') : '—' }}</span>
                            </td>
                            <td class="px-4 py-3">
                                <div class="flex items-center justify-end gap-1">
                                    @if ($item->status->value === 'draft')
                                        @can('publish', \App\Domains\News\Models\NewsItem::class)
                                            <button wire:click="publish({{ $item->id }})" class="p-2 rounded-lg hover:bg-success/10 text-text-tertiary hover:text-success transition-all" title="نشر">
                                    ...
                                    @if ($item->status->value === 'published')
                                        @can('publish', \App\Domains\News\Models\NewsItem::class)
                                            <button wire:click="unpublish({{ $item->id }})" class="p-2 rounded-lg hover:bg-warning/10 text-text-tertiary hover:text-warning transition-all" title="إلغاء النشر">
                                                <i data-lucide="x-circle" class="w-4 h-4"></i>
                                            </button>
                                        @endcan
                                    @endif
                                    @can('update', \App\Domains\News\Models\NewsItem::class)
                                        <a href="{{ route('dashboard.news.edit', $item->id) }}" class="p-2 rounded-lg hover:bg-municipal-50 text-text-tertiary hover:text-primary transition-all" wire:navigate title="تعديل">
                                            <i data-lucide="edit-3" class="w-4 h-4"></i>
                                        </a>
                                    @endcan
                                    @can('delete', \App\Domains\News\Models\NewsItem::class)
                        ...
                        <tr>
                            <td colspan="7" class="px-4 py-12 text-center">
                                <div class="flex flex-col items-center gap-2">
                                    <i data-lucide="newspaper" class="w-8 h-8 text-text-tertiary"></i>
                                    <p class="text-sm text-text-tertiary">لا توجد أخبار بعد</p>
                                    @can('create', \App\Domains\News\Models\NewsItem::class)
                                        <a href="{{ route('dashboard.news.create') }}" class="text-sm text-primary font-semibold hover:underline" wire:navigate>إضافة خبر جديد</a>
                                    @endcan
                                </div>
                            </td>
                        </tr>
    ...
    @if ($showDeleteModal)
        <div class="fixed inset-0 z-50 flex items-center justify-center bg-black/30 backdrop-blur-sm">
            <div class="bg-surface rounded-2xl border border-border p-6 w-full max-w-sm mx-4 shadow-dropdown">
                ...
                <p class="text-sm text-text-secondary mb-6">هل أنت متأكد من حذف هذا الخبر؟</p>
