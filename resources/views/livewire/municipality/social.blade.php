<div>
    <x-slot name="title">منصات التواصل الاجتماعي</x-slot>

    <div class="flex items-center justify-between mb-6">
        <div>
            <h1 class="text-2xl font-bold text-text">منصات التواصل الاجتماعي</h1>
            <p class="text-sm text-text-tertiary mt-1">إدارة روابط منصات التواصل الاجتماعي</p>
        </div>
        @can('createSocial', App\Domains\Municipality\Models\Municipality::class)
        ...
        @if ($platforms->isEmpty())
            <div class="text-center py-16">
                <div class="w-16 h-16 rounded-2xl bg-surface-secondary flex items-center justify-center mx-auto mb-3">
                    <i data-lucide="share-2" class="w-7 h-7 text-text-muted"></i>
                </div>
                <p class="text-sm font-bold text-text">لا توجد منصات تواصل</p>
                <p class="text-xs text-text-tertiary mt-1">أضف منصة تواصل جديدة للبدء.</p>
            </div>
        @else
            <div class="overflow-x-auto">
                <table class="w-full">
                    ...
                    <tbody>
                        ...
                        <tr wire:key="platform-{{ $platform->id }}" class="border-b border-border last:border-0 hover:bg-background/30 transition-colors">
                            <td class="px-6 py-4">
                                <div class="flex items-center gap-3">
                                    ...
                                    <div>
                                        ...
                                        <p class="text-[11px] text-text-tertiary">{{ $slugOptions[$platform->slug] ?? $platform->slug }}</p>
                                    </div>
                                </div>
                            </td>
                            <td class="px-6 py-4 text-sm text-text-secondary max-w-[200px] truncate">
                                <a href="{{ $platform->url }}" target="_blank" class="hover:text-primary transition-colors">{{ $platform->url }}</a>
                            </td>
                            <td class="px-6 py-4">
                            ...
                            <td class="px-6 py-4 text-sm text-text-secondary">{{ $platform->display_order }}</td>
                            <td class="px-6 py-4">
                                <div class="flex items-center gap-1">
                                    @can('updateSocial', App\Domains\Municipality\Models\Municipality::class)
                                    ...
                                    @endcan
                                    @can('deleteSocial', App\Domains\Municipality\Models\Municipality::class)
            ...
            <form wire:submit="save" class="p-5 space-y-4">
                <div class="grid grid-cols-2 gap-4">
                    <div>
                        ...
                        <input type="text" wire:model="name" class="w-full bg-surface-secondary border border-border rounded-xl px-4 py-2.5 text-sm text-text focus:border-primary focus:ring-1 focus:ring-primary/20 outline-none transition-all @error('name') border-danger @enderror" placeholder="مثال: فيسبوك" />
                        @error('name') <p class="mt-1 text-xs text-danger">{{ $message }}</p> @enderror
                    </div>
                    <div>
                        <label class="block text-sm font-semibold text-text mb-1.5">المعرف <span class="text-danger">*</span></label>
                        <select wire:model="slug" class="w-full bg-surface-secondary border border-border rounded-xl px-4 py-2.5 text-sm text-text focus:border-primary focus:ring-1 focus:ring-primary/20 outline-none transition-all @error('slug') border-danger @enderror">
                            @foreach ($slugOptions as $val => $label)
                ...
                <div class="grid grid-cols-2 gap-4">
                    <div>
                        ...
                        <input type="text" wire:model="icon" class="w-full bg-surface-secondary border border-border rounded-xl px-4 py-2.5 text-sm text-text focus:border-primary focus:ring-1 focus:ring-primary/20 outline-none transition-all @error('icon') border-danger @enderror" placeholder="facebook" />
                    ...
                    <div>
                        ...
                        <input type="text" wire:model="color" class="w-full bg-surface-secondary border border-border rounded-xl px-4 py-2.5 text-sm text-text focus:border-primary focus:ring-1 focus:ring-primary/20 outline-none transition-all @error('color') border-danger @enderror" placeholder="#1877F2" />
                ...
                <div>
                    ...
                    <input type="url" wire:model="url" class="w-full bg-surface-secondary border border-border rounded-xl px-4 py-2.5 text-sm text-text focus:border-primary focus:ring-1 focus:ring-primary/20 outline-none transition-all @error('url') border-danger @enderror" placeholder="https://facebook.com/idna" />
    ...
    <div class="fixed inset-0 z-50 flex items-center justify-center p-4">
        ...
        <div class="relative bg-surface rounded-2xl shadow-xl border border-border w-full max-w-md">
            <div class="p-6 text-center">
                ...
                <p class="text-sm text-text-tertiary">هل أنت متأكد من حذف منصة التواصل هذه؟</p>
            </div>
