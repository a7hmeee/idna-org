...
<section id="municipality-intro" class="bg-white">
    <div class="py-[48px] md:py-[60px] lg:py-[80px] container-home">
        ...
        <div class="grid grid-cols-1 lg:grid-cols-[minmax(0,46%)_minmax(0,54%)] lg:gap-12 lg:items-center">
            ...
            {{-- IMAGE / VISUAL (first track → right in RTL) --}}
            <div class="relative">
                <div class="rounded-[22px] overflow-hidden shadow-[0_16px_40px_rgba(25,60,35,0.12)] {{ $isPng ? 'bg-[#EAF5EE]' : 'bg-[#F4F5F1]' }} h-[260px] sm:h-[320px] lg:h-[420px]">
                    @if ($mainImgUrl)
                        <img src="{{ $mainImgUrl }}"
                             alt="{{ $mainImgAlt }}"
            ...
            {{-- CONTENT (second track → left in RTL) --}}
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-6">
                        @if ($hasVision)
                            <div class="rounded-2xl border border-[#DCE8DE] bg-gradient-to-br from-white to-[#F8FBF8] p-5 shadow-[0_4px_14px_rgba(20,55,30,0.035)]">
                                ...
                                <p class="text-[14px] text-[#66756D] leading-[1.8] line-clamp-3">{{ $municipality['vision'] }}</p>
                            </div>
                        @endif
                        @if ($hasMission)
                            <div class="rounded-2xl border border-[#DCE8DE] bg-gradient-to-br from-white to-[#F8FBF8] p-5 shadow-[0_4px_14px_rgba(20,55,30,0.035)]">
                                ...
                                <p class="text-[14px] text-[#66756D] leading-[1.8] line-clamp-3">{{ $municipality['mission'] }}</p>
                            </div>
                        @endif
                    </div>
                @endif
