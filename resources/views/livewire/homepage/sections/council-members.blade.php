    ...
    <div class="relative z-10 mx-auto w-full max-w-[1400px] px-5 sm:px-8">
        <header class="relative mb-10 flex flex-col items-center text-center">
            ...
            <h2 class="text-[clamp(28px,3.4vw,42px)] font-extrabold leading-tight tracking-tight text-[#17243A]">
                {{ $sectionTitle ?? 'أعضاء المجلس البلدي' }}
            </h2>
            <p class="mt-3 max-w-xl text-[clamp(14px,1.1vw,17px)] leading-relaxed text-[#64748B]">
                {{ $sectionSubtitle ?? 'نعمل معاً من أجل تطوير مدينتنا وخدمة المواطنين' }}
            </p>
        ...
        @if ($members->isNotEmpty())
            <div class="homepage-council-composition flex flex-col items-start gap-6 lg:flex-row" dir="ltr">
                    ...
                    <article class="homepage-council-president relative flex w-full shrink-0 flex-col overflow-hidden rounded-[28px] border border-[#0B4D28] bg-[#07552D] text-center text-white shadow-[0_18px_50px_rgba(8,70,34,.22)]" dir="rtl">
                        ...
                        <div class="relative z-10 flex min-h-0 flex-1 flex-col items-center p-5 sm:p-6">
                            <div class="flex h-[280px] w-full shrink-0 items-center justify-center overflow-hidden rounded-[20px] bg-[#0B4D28] ring-1 ring-white/15">
                                @if ($presidentPhoto)
                                    <img src="{{ $presidentPhoto }}" alt="{{ $presidentName }}" class="h-full w-full object-cover object-[center_22%]" loading="lazy" decoding="async">
                                @else
                                    <span class="text-5xl font-black text-white/35" aria-hidden="true">{{ $initials($presidentName) }}</span>
                                @endif
                            </div>
                            ...
                            <h3 class="mt-3 text-[clamp(20px,2vw,28px)] font-extrabold leading-tight">{{ $presidentName }}</h3>
                            @if (! empty($president['position_label'] ?? null))
                                <p class="mt-2 text-sm font-semibold text-white/75">{{ $president['position_label'] }}</p>
                            @endif
                            @if (! empty($president['bio'] ?? null))
                                <p class="mt-2 text-[13px] leading-[1.8] text-white/60">{{ $president['bio'] }}</p>
                            @endif

                            <div class="mt-auto flex items-center justify-center gap-3 pt-4">
                                @if (! empty($president['phone'] ?? $president['mobile'] ?? null))
                                    <a href="tel:{{ $president['phone'] ?? $president['mobile'] }}" class="flex h-10 w-10 items-center justify-center rounded-full border border-white/20 bg-white/10 text-white transition hover:border-[#C8A85A] hover:bg-[#C8A85A]" aria-label="اتصال بـ {{ $presidentName }}"><i data-lucide="phone" class="h-4 w-4"></i></a>
                                @endif
                                @if (! empty($president['email'] ?? null))
                                    <a href="mailto:{{ $president['email'] }}" class="flex h-10 w-10 items-center justify-center rounded-full border border-white/20 bg-white/10 text-white transition hover:border-[#C8A85A] hover:bg-[#C8A85A]" aria-label="بريد {{ $presidentName }}"><i data-lucide="mail" class="h-4 w-4"></i></a>
                                @endif
                                @if ($presidentProfile !== '#')
                                    <a href="{{ $presidentProfile }}" wire:navigate class="flex h-10 w-10 items-center justify-center rounded-full border border-white/20 bg-white/10 text-white transition hover:border-[#C8A85A] hover:bg-[#C8A85A]" aria-label="الملف الشخصي لـ {{ $presidentName }}"><i data-lucide="user" class="h-4 w-4"></i></a>
                                @endif
                            </div>
                        </div>
                    </article>
                ...
                @if ($regularMembers->isNotEmpty())
                    @php $sliderData = $regularMembers->all(); @endphp
                    <div class="homepage-council-slider min-w-0 flex-1" x-data="{
                        slides: @js($sliderData),
                        slider: null,
                        currentPage: 0,
                        canPrev: false,
                        canNext: false,
                                    ...
                                    <div class="homepage-council-slide flex shrink-0 snap-start" dir="rtl">
                                        <article class="homepage-council-card flex w-full flex-col overflow-hidden rounded-[22px] border border-[#E9EFEA] bg-white text-center shadow-[0_6px_20px_rgba(15,23,42,.05)] transition duration-300 hover:-translate-y-1 hover:shadow-[0_14px_36px_rgba(15,23,42,.1)]">
                                            <div class="flex h-[230px] w-full shrink-0 items-center justify-center overflow-hidden rounded-t-[18px] bg-[#F0F7F2] p-4">
                                                @if ($image)
                                                    <img src="{{ $image }}" alt="{{ $name }}" class="h-full w-full rounded-[14px] object-cover object-[center_25%]" loading="lazy" decoding="async">
                                                @else
                                                    <span class="text-4xl font-black text-[#176B32]/25" aria-hidden="true">{{ $initials($name) }}</span>
                                                @endif
                                            </div>
                                            <div class="flex flex-1 flex-col px-5 py-4">
                                                <h3 class="text-[17px] font-extrabold leading-snug text-[#17243A]">{{ $name }}</h3>
                                                @if (! empty($member['position_label'] ?? null))
                                                    <p class="mt-1.5 text-[13px] font-semibold text-[#176B32]">{{ $member['position_label'] }}</p>
                                                @endif
                                                @if (! empty($member['committee'] ?? null))
                                                    <span class="mt-1 text-[12px] leading-relaxed text-[#64748B]">{{ $member['committee'] }}</span>
                                                @endif
                                                @if (! empty($member['bio'] ?? null))
                                                    <p class="mt-2 text-[12px] leading-[1.8] text-[#64748B]">{{ $member['bio'] }}</p>
                                                @endif
                                                <div class="mt-auto flex items-center justify-center gap-3 pt-4">
                                                    @if (! empty($member['phone'] ?? $member['mobile'] ?? null))
                                                        <a href="tel:{{ $member['phone'] ?? $member['mobile'] }}" class="flex h-9 w-9 items-center justify-center rounded-full bg-[#EEF8F0] text-[#176B32] transition hover:bg-[#176B32] hover:text-white" aria-label="اتصال بـ {{ $name }}"><i data-lucide="phone" class="h-4 w-4"></i></a>
                                                    @endif
                                                    @if (! empty($member['email'] ?? null))
                                                        <a href="mailto:{{ $member['email'] }}" class="flex h-9 w-9 items-center justify-center rounded-full bg-[#EEF8F0] text-[#176B32] transition hover:bg-[#176B32] hover:text-white" aria-label="بريد {{ $name }}"><i data-lucide="mail" class="h-4 w-4"></i></a>
                                                    @endif
                                                    @if ($url !== '#')
                                                        <a href="{{ $url }}" wire:navigate class="flex h-9 w-9 items-center justify-center rounded-full bg-[#EEF8F0] text-[#176B32] transition hover:bg-[#176B32] hover:text-white" aria-label="الملف الشخصي لـ {{ $name }}"><i data-lucide="user" class="h-4 w-4"></i></a>
                                                    @endif
                                                </div>
                                            </div>
                                        </article>
        ...
        <div class="mt-8 rounded-2xl border border-[#E8EDF2] bg-[#F8FAF9] px-6 py-5" dir="rtl">
            <div class="flex flex-col items-center justify-center gap-5 divide-y divide-[#E2E8F0] sm:flex-row sm:divide-x sm:divide-y-0">
                <div class="flex items-center gap-3 px-5 py-2">
                    ...
                    <div><strong class="block text-2xl font-extrabold text-[#17243A]">{{ $members->count() }}</strong><span class="text-xs text-[#64748B]">أعضاء المجلس</span></div>
                </div>
                <div class="flex items-center gap-3 px-5 py-2">
                ...
                <div class="flex items-center gap-3 px-5 py-2">
                    ...
                    <div><strong class="block text-base font-bold text-[#17243A]">{{ $municipalityName ?? '' }}</strong><span class="text-xs text-[#64748B]">شفافية · كفاءة · تطوير</span></div>
                </div>
            </div>
        </div>
        ...
        @if (Route::has('public.council.index'))
            <div class="mt-5 flex justify-center lg:hidden">
                <a href="{{ route('public.council.index') }}" wire:navigate class="inline-flex h-11 items-center gap-2 rounded-xl border border-[#176B32] px-5 text-sm font-bold text-[#176B32] no-underline transition hover:bg-[#176B32] hover:text-white">
