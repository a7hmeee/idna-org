{{-- ============================================================
     ...
     Static, presentation-only strip. Existing routes/icons only.
     ============================================================ --}}
@php
    $quickActions = is_array($quickLinks ?? null) ? collect($quickLinks ?? []) : ($quickLinks ?? collect());
    $hasQuickActions = $quickActions->isNotEmpty();
@endphp
    ...
    <div class="container-home py-10 sm:py-12">
        ...
        <nav aria-label="إجراءات سريعة للمواطنين">
            <div class="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-4 sm:gap-5">
                @if ($hasQuickActions)
                    @foreach ($quickActions->where('is_active', true)->sortBy('sort_order') as $action)
                        <a href="{{ $action->url }}"
                           @if ($action->is_external) target="_blank" rel="noopener noreferrer" @endif
                           wire:navigate
                           class="group flex flex-col items-center gap-3 rounded-2xl border border-border bg-surface px-4 py-6 text-center no-underline transition-all duration-200 hover:border-primary/30 hover:shadow-md hover:-translate-y-0.5 focus-visible:ring-2 focus-visible:ring-primary/40 focus-visible:outline-none">
                            <span class="w-14 h-14 rounded-xl bg-primary-light text-primary flex items-center justify-center transition-colors group-hover:bg-primary group-hover:text-white">
                                <i data-lucide="{{ $action->icon ?? 'link' }}" class="w-7 h-7"></i>
                            </span>
                            <span class="text-[14px] font-bold text-text group-hover:text-primary transition-colors">{{ $action->title }}</span>
                        </a>
                    @endforeach
                @else
                    @if (Route::has('public.complaints.submit'))
                        <a href="{{ route('public.complaints.submit') }}" wire:navigate
                           class="group flex flex-col items-center gap-3 rounded-2xl border border-border bg-surface px-4 py-6 text-center no-underline transition-all duration-200 hover:border-primary/30 hover:shadow-md hover:-translate-y-0.5 focus-visible:ring-2 focus-visible:ring-primary/40 focus-visible:outline-none">
                            <span class="w-14 h-14 rounded-xl bg-primary-light text-primary flex items-center justify-center transition-colors group-hover:bg-primary group-hover:text-white">
                    ...
                    @endif
                    @if (Route::has('public.complaints.track'))
                        <a href="{{ route('public.complaints.track') }}" wire:navigate
                           class="group flex flex-col items-center gap-3 rounded-2xl border border-border bg-surface px-4 py-6 text-center no-underline transition-all duration-200 hover:border-primary/30 hover:shadow-md hover:-translate-y-0.5 focus-visible:ring-2 focus-visible:ring-primary/40 focus-visible:outline-none">
                            <span class="w-14 h-14 rounded-xl bg-primary-light text-primary flex items-center justify-center transition-colors group-hover:bg-primary group-hover:text-white">
