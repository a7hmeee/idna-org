<div>
    <x-slot name="title">شرائح البنر</x-slot>
    ...
    <div class="flex items-center justify-between mb-6">
        ...
        @can('createSlide', \App\Domains\Homepage\Models\HomepageSetting::class)
            <a href="{{ route('dashboard.homepage.slides.create') }}" class="px-4 py-2.5 rounded-xl bg-primary text-white text-sm font-semibold hover:bg-primary-dark transition-colors inline-flex items-center gap-2" wire:navigate>
                    ...
                    @forelse ($slides as $slide)
                        <tr class="border-b border-border last:border-0 hover:bg-municipal-50/30 transition-colors">
                            ...
                            <td class="px-4 py-3">
                                ...
                                @if ($slide->badge_text)
                                    <span class="text-[10px] bg-primary/10 text-primary px-2 py-0.5 rounded-full font-medium">{{ $slide->badge_text }}</span>
                                @endif
                            </td>
                            <td class="px-4 py-3">
                            ...
                            <td class="px-4 py-3">
                                <div class="flex items-center justify-end gap-2">
                                    @can('updateSlide', \App\Domains\Homepage\Models\HomepageSetting::class)
                                        <a href="{{ route('dashboard.homepage.slides.edit', $slide->id) }}" class="p-2 rounded-lg hover:bg-municipal-50 text-text-tertiary hover:text-primary transition-all" wire:navigate>
                        ...
                        <tr>
                            <td colspan="5" class="px-4 py-12 text-center">
                                <div class="flex flex-col items-center gap-2">
                                    ...
                                    @can('createSlide', \App\Domains\Homepage\Models\HomepageSetting::class)
                                        <a href="{{ route('dashboard.homepage.slides.create') }}" class="text-sm text-primary font-semibold hover:underline" wire:navigate>إضافة شريحة جديدة</a>
                                    @endcan
                                </div>
                            </td>
                        </tr>
