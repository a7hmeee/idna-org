...
<input type="text" wire:model.live.debounce.400ms="search" aria-label="ابحث عن وظيفة"
            ...
            <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:20px;">
                <p style="font-size:13px;color:#6B7280;margin:0;">
                    يوجد <span style="font-weight:700;color:#1F2937;">{{ $jobs->total() ?? 0 }}</span> وظيفة
                </p>
            </div>
                ...
                <div style="margin-bottom:28px;">
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                        @foreach ($featured as $job)
                            <a href="{{ route('public.jobs.show', $job->slug) }}" wire:navigate
                               ...
                               onmouseout="this.style.borderColor='#FEF3C7';this.style.boxShadow='0 1px 3px rgba(0,0,0,0.03)';this.style.transform='translateY(0)'">
                                <div style="display:flex;align-items:flex-start;gap:14px;margin-bottom:10px;">
                                    ...
                                    <div style="min-width:0;flex:1;">
                                        <h2 style="font-size:14px;font-weight:700;color:#1F2937;margin:0 0 2px;">{{ $job->title }}</h2>
                                        @if ($job->department)
                                            <span style="font-size:11px;color:#9CA3AF;">{{ $job->department->name }}</span>
                                        @endif
                                    </div>
                                </div>
                                <p style="font-size:12px;color:#9CA3AF;line-height:1.6;margin:0 0 10px;overflow:hidden;display:-webkit-box;-webkit-line-clamp:2;-webkit-box-orient:vertical;">{{ $job->summary }}</p>
            ...
            @if ($jobs->isEmpty())
                <div style="text-align:center;padding:64px 24px;background:white;border-radius:16px;border:1px solid #F3F4F6;">
                    ...
                    <h2 style="font-size:16px;font-weight:700;color:#1F2937;margin:0 0 8px;">لا توجد وظائف شاغرة حالياً</h2>
                    <p style="font-size:13px;color:#9CA3AF;margin:0;">جرّب البحث بكلمات مختلفة أو غيّر التصفية</p>
                </div>
            @else
                <div class="space-y-3">
                    @foreach ($jobs as $job)
                        <a href="{{ route('public.jobs.show', $job->slug) }}" class="block bg-white rounded-xl border border-gray-200 p-5 hover:shadow-sm transition-shadow" wire:navigate>
                            <div class="flex items-start justify-between gap-3">
                                <div class="flex-1">
                                    <h2 class="font-bold text-text">{{ $job->title }}</h2>
                                    <p class="text-sm text-text-secondary mt-1 line-clamp-2">{{ $job->summary }}</p>
                                    <div class="flex items-center gap-4 mt-3 text-xs text-text-tertiary flex-wrap">
                                        <span class="inline-flex items-center gap-1">
                                            <i data-lucide="building-2" class="w-3 h-3"></i>
                                            {{ $job->department?->name ?? 'بلدية إذنا' }}
                                        </span>
                                        <span class="inline-flex items-center gap-1">
                                        ...
                                        <span class="inline-flex items-center gap-1">
                                            ...
                                            آخر موعد: {{ $job->closing_at->format('Y/m/d') }}
                                        </span>
                                    </div>
                                </div>
                                <div class="shrink-0 flex flex-col items-end gap-2">
                                    <span class="text-xs bg-primary/10 text-primary px-2.5 py-1 rounded-full font-semibold">{{ $job->employment_type->label() }}</span>
                                    @if ($job->is_featured)
