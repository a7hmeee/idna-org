<div>
    <x-slot name="title">تسميات حالات المياه</x-slot>

    <div class="flex items-center justify-between mb-6">
        <div>
            <h1 class="text-2xl font-bold text-text">تسميات حالات المياه</h1>
            <p class="text-sm text-text-tertiary mt-1">إدارة تسميات وألوان حالات المياه</p>
        </div>
    </div>
            ...
            <table class="w-full text-sm">
                <thead>
                    <tr class="border-b border-border bg-municipal-50/50">
                        ...
                        <th class="text-center px-4 py-3 text-xs font-semibold text-text-tertiary">لون النص</th>
                        <th class="text-center px-4 py-3 text-xs font-semibold text-text-tertiary">لون الخلفية</th>
                        <th class="text-center px-4 py-3 text-xs font-semibold text-text-tertiary">لون النقطة</th>
                ...
                <tbody>
                    @forelse ($labels as $label)
                        <tr class="border-b border-border last:border-0 hover:bg-municipal-50/30 transition-colors" wire:key="label-{{ $label->id }}">
                            <td class="px-4 py-3">
                                <span class="font-mono text-xs text-text-tertiary">{{ $label->status_key }}</span>
                            </td>
                            <td class="px-4 py-3">
                                <div class="flex items-center gap-2">
                                    <span class="inline-block w-3 h-3 rounded-full shrink-0" style="background-color: {{ $label->dot_color }}"></span>
                                    <span class="font-semibold text-text">{{ $label->label_ar }}</span>
                                </div>
                            </td>
                            <td class="px-4 py-3 text-center">
                                <span class="inline-block w-6 h-6 rounded-lg border border-border" style="background-color: {{ $label->color }}"></span>
                            </td>
                            <td class="px-4 py-3 text-center">
                                <span class="inline-block w-6 h-6 rounded-lg border border-border" style="background-color: {{ $label->bg_color }}"></span>
                            </td>
                            <td class="px-4 py-3 text-center">
                                <span class="inline-block w-3 h-3 rounded-full" style="background-color: {{ $label->dot_color }}"></span>
                            </td>
                            <td class="px-4 py-3 text-center">
                        ...
                        <tr>
                            <td colspan="7" class="px-4 py-12 text-center">
                                <div class="flex flex-col items-center gap-3">
                                    <i data-lucide="droplets" class="w-12 h-12 text-text-tertiary/40"></i>
                                    <p class="text-sm text-text-tertiary">لا توجد تسميات حالات</p>
                                </div>
                            </td>
                        </tr>
                    @endforelse
            ...
            <div class="bg-surface rounded-2xl border border-border p-6 w-full max-w-md mx-4 shadow-dropdown">
                <div class="flex items-center gap-3 mb-6">
                    <div class="w-10 h-10 rounded-xl bg-primary/10 flex items-center justify-center">
                        <i data-lucide="droplets" class="w-5 h-5 text-primary"></i>
                    </div>
                    <div>
                        <h3 class="font-semibold text-text">تعديل تسمية الحالة</h3>
                        <p class="text-xs text-text-tertiary">تحديث معلومات تسمية الحالة</p>
                    </div>
                </div>
                ...
                <div class="space-y-4">
                    ...
                    <div class="grid grid-cols-3 gap-4">
                        <div>
                            <label class="block text-xs font-semibold text-text-tertiary mb-1.5">لون النص</label>
                            <div class="flex items-center gap-2">
                                <input type="color" wire:model="color" class="w-8 h-8 rounded-lg border border-border cursor-pointer" />
                                <input type="text" wire:model="color" class="flex-1 bg-surface-secondary border border-border rounded-xl px-3 py-2 text-xs text-text focus:border-primary outline-none transition-all" dir="ltr" />
                            </div>
                        </div>
                        <div>
                            <label class="block text-xs font-semibold text-text-tertiary mb-1.5">لون الخلفية</label>
                            <div class="flex items-center gap-2">
                                <input type="color" wire:model="bgColor" class="w-8 h-8 rounded-lg border border-border cursor-pointer" />
                                <input type="text" wire:model="bgColor" class="flex-1 bg-surface-secondary border border-border rounded-xl px-3 py-2 text-xs text-text focus:border-primary outline-none transition-all" dir="ltr" />
                            </div>
                        </div>
                        <div>
                            <label class="block text-xs font-semibold text-text-tertiary mb-1.5">لون النقطة</label>
                            <div class="flex items-center gap-2">
                                <input type="color" wire:model="dotColor" class="w-8 h-8 rounded-lg border border-border cursor-pointer" />
                                <input type="text" wire:model="dotColor" class="flex-1 bg-surface-secondary border border-border rounded-xl px-3 py-2 text-xs text-text focus:border-primary outline-none transition-all" dir="ltr" />
                            </div>
                        </div>
                    </div>
