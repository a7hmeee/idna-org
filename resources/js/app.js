...
function refreshIcons(source) {
  console.log('[icons] refreshIcons source=' + source);
  if (iconFrame) cancelAnimationFrame(iconFrame);
  iconFrame = requestAnimationFrame(() => {
    console.log('[icons] rAF fired');
    replaceIcons();
  });
}

document.addEventListener('DOMContentLoaded', () => refreshIcons('DOMContentLoaded'));

document.addEventListener('livewire:navigated', () => refreshIcons('navigated'));

function registerHooks() {
  if (window.Livewire) {
    console.log('[icons] registering morph hooks');
    window.Livewire.hook('morph.added', () => refreshIcons('morph.added'));
    window.Livewire.hook('morph.updated', () => refreshIcons('morph.updated'));
    console.log('[icons] hooks registered');
  } else {
    console.warn('[icons] window.Livewire not available');
  }
}

console.log('[icons] app.js loaded, window.Livewire=' + !!window.Livewire);
if (window.Livewire) {
  registerHooks();
} else {
  console.log('[icons] Livewire not ready yet, waiting for events');
  document.addEventListener('livewire:init', () => {
    console.log('[icons] livewire:init fired');
    registerHooks();
  });
  document.addEventListener('livewire:initialized', () => {
    console.log('[icons] livewire:initialized fired');
    registerHooks();
  });
}
